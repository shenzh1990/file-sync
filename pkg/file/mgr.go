	...
	for !c.conn.IsClose() {
		...
		case model.MsgHead:
			...
			if err != nil {
				...
				c.conn.SendMsg(model.NewNotifyMsg(c.fileName, model.Status_Err))
				return err
			}
			fmt.Println("send head is ok")

			c.conn.SendMsg(model.NewNotifyMsg(c.fileName, model.Status_Ok))
		case model.MsgFile:
			if fs == nil {
				fmt.Println(c.fileName, "file is not open !")
				c.conn.SendMsg(model.NewCloseMsg(c.fileName, model.Status_Err))
				return nil
			}
			// 写入文件
		...
		case model.MsgEnd:
			...
			_ = fs.Close()
			fs = nil
		case model.MsgNotify:
			c.waitNotify <- m.Bytes[0] == byte(model.Status_Ok)
		case model.MsgClose:
			fmt.Printf("revc close msg ....\n")
			if m.Bytes[0] != byte(model.Status_Ok) {
...
func (c *ConMgr) sendFile() error {
	...
	c.conn.SendMsg(model.NewCloseMsg(c.fileName, model.Status_Ok))
	return err
}
...
func (c *ConMgr) sendSingleFile(filePath string) error {
	...
	m := model.NewHeadMsg(fileInfo.Name())
	// 发送文件信息
	c.conn.SendMsg(m)
	...
	for !c.conn.IsClose() {
		// 发送文件数据
		readBuf := model.BytesPool.Get().([]byte)
		...
		c.conn.SendMsg(model.NewFileMsg(c.fileName, readBuf[:n]))
	}

	c.conn.SendMsg(model.NewEndMsg(c.fileName, uint64(fileInfo.Size())))
